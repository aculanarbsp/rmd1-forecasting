...
class Modeling:
    ...
    def __init__(self, batch_size: int, max_epochs: int, n_steps: int, forecast_horizon: int, stateful: bool):
        ...
        for key in params.keys():
            ...
            if key == 'rnn':
                ...
                model = KerasRegressor(
                            ...
                            n_units=n_units,
                            epochs=self.max_epochs, 
                            batch_size=self.batch_size, loss=mean_absolute_error,
                            verbose=2
                        )
    ...
    def train(self, es: EarlyStopping, dataset_: str, model_: str, bayes: bool = False):
        ...
        if bayes==False:
            with open(f'data/processed/{model_}_{dataset_}_fh{self.forecast_horizon}.pkl', 'rb') as handle:
                params = pickle.load(handle)
        else:
            with open(f'data/processed/{model_}_{dataset_}_fh{self.forecast_horizon}_bayes.pkl', 'rb') as handle:
                params = pickle.load(handle)
        ...
        for key in params.keys():
            ...
            load = Load(dataset_=dataset_, forecast_horizon=self.forecast_horizon)
        ...
        for key in params.keys():
            ...
            if bayes == False:
                with open(f'models/{dataset_}_models_{key}_fh{self.forecast_horizon}_state{self.stateful}.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)

                with open(f'streamlit/pages/models/{dataset_}_models_{key}_fh{self.forecast_horizon}_state{self.stateful}.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)

            else:
                with open(f'models/{dataset_}_models_{key}__state{self.stateful}_bayes.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)

                with open(f'streamlit/pages/models/{dataset_}_models_{key}__state{self.stateful}_bayes.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)

    def cross_validate_bayesian(self, params: dict, es: EarlyStopping, dataset: str, forecast_horizon: int):
        load = Load(dataset_=dataset, forecast_horizon=forecast_horizon)
        train_test_val_dict = load.load_data()
        ...
        n_units = Integer(1,25, prior="uniform")
        l1_reg = Real(0, 0.001, prior="uniform")
        ...
        for key in params.keys():
            ...
            # save params
            with open(f'data/processed/{key}_{dataset}_fh{self.forecast_horizon}_bayes.pkl', 'wb') as handle:
                pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)
