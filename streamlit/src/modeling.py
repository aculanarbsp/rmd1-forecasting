...
class Modeling:
    ...
    def train(self, es: EarlyStopping, dataset_: str, model_: str, bayes: bool = False):
        ...
        for key in params.keys():
            ...
            if bayes == False:
                with open(f'models/{dataset_}_models_{key}_fh{self.forecast_horizon}.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)

                with open(f'streamlit/pages/models/{dataset_}_models_{key}_fh{self.forecast_horizon}.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)

            else:
                with open(f'models/{dataset_}_models_{key}_state{self.stateful}_bayes.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)

                with open(f'streamlit/pages/models/{dataset_}_models_{key}_state{self.stateful}_bayes.pkl', 'wb') as handle:
                    pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def cross_validate_bayesian(self, params: dict, es: EarlyStopping, dataset: str, forecast_horizon: int):
        ...
        for key in params.keys():
            ...
            # save params
            with open(f'data/processed/{key}_{dataset}_fh{self.forecast_horizon}_state{self.stateful}_bayes.pkl', 'wb') as handle:
                pickle.dump(params, handle, protocol=pickle.HIGHEST_PROTOCOL)
